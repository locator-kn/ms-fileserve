...
routes.push({
    ...
    handler: (request, reply) => {
        ...
        // test if an video format
        if (!regex.test(file.hapi.headers['content-type'])) {
            return reply(Boom.unsupportedMediaType('Only video format allowed'));
        }
        ...
        let pathToMovie = 'C:/Users/Steffen/git/locator/ms-fileserve/lib/impression.mp4';

        //ffmpeg(file).pipe(writestream, {end: true});
        // .format("mp4")
        //  .size("320x?")
        // .addOptions([
        ...
        ffmpeg.getAvailableFilters(function (err, formats) {
