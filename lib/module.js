...
routes.push({
    ...
    handler: (request, reply) => {
        var db = request.server.plugins['hapi-mongodb'].db;
        console.time('getting meta info for' + request.params.fileId);
        util.safeObjectId(request.params.fileId)
            .then(oId => {
                return d.b.collection('fs.files')
                    .find({_id: oId})
                    .limit(-1)
                    .next();
            })
        .then(result => {
            console.timeEnd('getting meta info for' + request.params.fileId);
            console.log(result);
            var gfs = new Grid(db, mongodb);
            ...
            reply(readstream).type('video/mp4').bytes(result.length);
        });
