        ...
        util.safeObjectId(request.params.fileId)
            ...
            .then(result => {
                ...
                if (request.params.ext === '3gp') {
                    return reply(readstream).type('video/3gp').bytes(result.length);
                }
...
routes.push({
    ...
    handler: (request, reply) => {
        ...
        // create a writestream for the db
        let xlargeWritestream = gfs.createWriteStream({
            filename: fileName,
            _id: xlargeId
        });
...
routes.push({
    ...
    handler: (request, reply) => {
        ...
        let regex = /^video\/(?:mp4|3gp|mpeg|mov|quicktime)$/;
